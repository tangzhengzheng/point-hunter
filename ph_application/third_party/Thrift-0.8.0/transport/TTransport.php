...
class TTransportException extends TException {
  ...
  function __construct($message=null, $code=0) {
 	...
 	$parts = explode('/', $_SERVER['PATH_INFO']);
  	$len = count($parts);
  	$path = $len >= 2 ? $parts[$len-2].'/'.$parts[$len-1] : $_SERVER['PATH_INFO'];
  	$errmsg = 'TTransportException['.$code.']:'.$message.', path:'.$path;
 	log_message('error', $errmsg);
//  log_with_mon_err($errmsg, MON_ERRCODE::MEC_TH_SOCKET);
